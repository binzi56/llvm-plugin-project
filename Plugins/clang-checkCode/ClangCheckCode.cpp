...
namespace
{
    ...
    private:
        ...
        void checkClassNameForLowercaseName(ObjCInterfaceDecl *decl)
        {
            ...
            {
                //修正提示
                std::string tempName = (std::string)className;
        ...
        void checkMethodNameForUppercaseName(ObjCMethodDecl *decl)
        {
            ...
            for (int i = 0; i < selectorPartCount; i++)
            {
                ...
                {
                    //修正提示
                    std::string tempName = (std::string)selName;
        ...
        void checkDelegatePropertyForUsageWeak (ObjCPropertyDecl *decl)
        {
            ...
            {
                ObjCPropertyAttribute::Kind attrKind = decl -> getPropertyAttributes();
                ...
                if(!(attrKind & ObjCPropertyAttribute::kind_weak))
                {
                    DiagnosticsEngine &diagEngine = Instance.getDiagnostics();
    ...
    protected:
        ...
        {
            return std::make_unique<CodeConsumer>(CI, ParsedTemplates);
        }
...
X("clang-checkCode", "Code Checker");
